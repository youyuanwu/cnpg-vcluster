...
| Calico | `v3.32.2` manifest | `manifests/addons/calico.yaml` is a deterministic transform with the lab CIDR and recorded image digests. |
| Local Path Provisioner | `v0.0.37` manifest | `manifests/addons/local-path.yaml` is a deterministic transform with the lab storage path/default-class settings and recorded image digests. |
| CloudNativePG | `v1.30.0` release manifest | `manifests/cnpg/operator.yaml` is byte-identical to the verified release; installation substitutes the approved controller digest in memory. |
