...
write_management_network() {
  ...
  selected="$(
    ...
    EXCLUDED_CIDRS="${MANAGEMENT_POD_CIDR} ${MANAGEMENT_SERVICE_CIDR} ${TENANT_A_POD_CIDR} ${TENANT_A_SERVICE_CIDR} ${TENANT_B_POD_CIDR} ${TENANT_B_SERVICE_CIDR} ${SPIKE_POD_CIDR} ${SPIKE_SERVICE_CIDR}" \
    python3 -c '
import ipaddress
import json
import os
