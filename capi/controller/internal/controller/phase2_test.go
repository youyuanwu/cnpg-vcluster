...
type staticTenantClientFactory struct {
	client client.Client
}

func (factory staticTenantClientFactory) ClientFor([]byte, string) (client.Client, error) {
	return factory.client, nil
}

func (fake *fakeDockerClient) InspectContainer(context.Context, string) (DockerContainer, error) {
...
func testFoundation() Foundation {
	return Foundation{
		...
		Cache: FoundationCache{
			...
			ImageArchives: []FoundationArchive{{
				Key:       "VERIFY_IMAGE",
				Path:      "images/verify_image.tar",
				SHA256:    strings.Repeat("b", 64),
				Reference: "busybox:1@sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
				Tagged:    "busybox:1",
				Worker:    true,
			}},
		},
...
func TestLoadFoundationUsesLiveDockerIdentity(t *testing.T) {
	...
	observed, err := loadFoundation(context.Background(), kubernetes, docker, defaultFoundationNamespace, defaultFoundationName, "1.36.4")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := loadFoundation(context.Background(), kubernetes, docker, defaultFoundationNamespace, defaultFoundationName, "1.36.4"); err == nil {
...
func TestFoundationRejectsMalformedCacheAndCIDRs(t *testing.T) {
	...
	if err := validateFoundation(foundation, "1.36.4"); err == nil {
...
func TestWorkerPreparationVerifiesCacheMirrorEgressAndPull(t *testing.T) {
	...
	reconciler := &TenantReconciler{Docker: docker}
	if err := reconciler.prepareWorker(context.Background(), DockerContainer{ID: "worker-id", Name: "worker"}, foundation); err != nil {
		t.Fatal(err)
	}
...
func TestWorkerPreparationStopsOnCacheMismatch(t *testing.T) {
	...
	reconciler := &TenantReconciler{Docker: docker}
	if err := reconciler.prepareWorker(context.Background(), DockerContainer{ID: "worker-id"}, foundation); err == nil {
...
func TestPartialFinalizationReleasesEndpointAfterStatusCrashGap(t *testing.T) {
	...
	if !result.Requeue {
		t.Fatal("endpoint release did not request a persisted follow-up")
	}
	...
	if updated.Status.Stage != "EndpointReleased" {
		t.Fatalf("unexpected finalization stage: %s", updated.Status.Stage)
	}
}
...
func TestPartialFinalizationDeletesUnrecordedExactVolume(t *testing.T) {
	...
	tenant := &tenancyv1alpha1.Tenant{
		...
		Status: tenancyv1alpha1.TenantStatus{
			Stage: tenancyv1alpha1.StageBootstrapRBACApplied,
	...
	secret := &corev1.Secret{
		...
		Data: map[string][]byte{"value": []byte("ignored by test factory")},
	}
	kubernetes := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(tenant).WithObjects(tenant, secret).Build()
	tenantScheme := runtime.NewScheme()
	if err := corev1.AddToScheme(tenantScheme); err != nil {
		t.Fatal(err)
	}
	if err := rbacv1.AddToScheme(tenantScheme); err != nil {
		t.Fatal(err)
	}
	...
	reconciler := &TenantReconciler{
		...
		Docker:        docker,
		TenantClients: staticTenantClientFactory{client: tenantClient},
	}
	result, err := reconciler.finalizePartial(context.Background(), tenant, "spec-hash", foundation)
	if err != nil {
		t.Fatal(err)
	...
	if !result.Requeue {
		t.Fatal("volume removal did not request a follow-up")
	}
