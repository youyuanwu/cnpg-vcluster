...
func TestDeleteIsAllowedWithoutReservation(t *testing.T) {
	...
	response := handler.Handle(context.Background(), admission.Request{AdmissionRequest: admissionv1.AdmissionRequest{
		...
		OldObject: runtimeRaw(tenantJSON("1.36.4", "")),
	}})
	if !response.Allowed {
		t.Fatalf("ordinary delete was denied: %s", response.Result.Message)
	}
}
