...
from scripts.break_glass import (
    FINALIZERS,
    _docker_inventory,
    _verify_resource_graph,
    break_glass,
)
...
class BreakGlassTests(unittest.TestCase):
    ...
    def test_graph_rejects_copied_label_cluster(self) -> None:
        ...
        with patch(
            "scripts.break_glass.verify_tenant_management_ownership",
            return_value={
                "cluster": {
                    "metadata": {"name": "tenant-a", "uid": "owned"}
                }
            },
        ):
    ...
    def test_graph_rejects_wrong_exact_resource_name(self) -> None:
        tenant = type("Tenant", (), {"name": "tenant-a"})()
        resource = {
            "apiVersion": "cluster.x-k8s.io/v1beta2",
        ...
        with patch(
            ...
            with self.assertRaisesRegex(RuntimeError, "outside tenant graph"):
                _verify_resource_graph(
                    {
                        "OWNERSHIP_LABEL": "example.owner",
                        "LAB_PREFIX": "lab",
    ...
    def test_docker_inventory_is_scoped_to_target_tenant(self) -> None:
        tenant = type("Tenant", (), {"name": "tenant-a"})()
        with (
            patch(
                "scripts.break_glass.inspect_storage_volume",
                return_value=None,
            ),
            patch(
                "scripts.break_glass.run",
                return_value=result(0, stdout="worker-id\n"),
            ) as run,
        ):
            inventory = _docker_inventory(
                {"LAB_PREFIX": "lab"}, tenant
            )
        self.assertEqual(inventory["containers"], ["worker-id"])
        self.assertIn(
            "label=io.x-k8s.kind.cluster=tenant-a",
            run.call_args.args[0],
        )

    def test_devmachine_graph_requires_api_group_and_machine_owner(self) -> None:
        tenant = type("Tenant", (), {"name": "tenant-a"})()
        machine = {
            "metadata": {"name": "worker", "uid": "machine-uid"},
            "spec": {
                "infrastructureRef": {
                    "apiGroup": "foreign.example",
                    "kind": "DevMachine",
                    "name": "worker",
                }
            },
