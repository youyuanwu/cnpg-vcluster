...
This repository creates a local two-tenant CloudNativePG environment on one
Docker host:
...
All other command-line tools are downloaded into `.tools/` with pinned
checksums.
...
2. Log in as `admin`; the generated password is in
   `.runtime/credentials/platform-admin-password`.
...
`.runtime/` with restrictive permissions. They are ignored by Git.
